...
__device__ void md5_hash(const uint8_t* data, uint32_t len, uint32_t* out4) {
  if (len > 55) {
    // Multi-block not implemented - return zero hash
...
__device__ inline void sha1_hash(const uint8_t* data, uint32_t len, uint32_t* out5) {
  if (len > 55) {
    // Multi-block not implemented - return zero hash
  ...
  uint8_t block[64];
  for (uint32_t i = 0; i < 64; ++i) block[i] = 0;
  for (uint32_t i = 0; i < len && i < 55; ++i) block[i] = data[i];
  block[len] = 0x80u;
  
  uint64_t bits = (uint64_t)len * 8u;
  block[56] = (bits >> 56) & 0xffu;
  block[57] = (bits >> 48) & 0xffu;
  block[58] = (bits >> 40) & 0xffu;
  block[59] = (bits >> 32) & 0xffu;
  block[60] = (bits >> 24) & 0xffu;
  block[61] = (bits >> 16) & 0xffu;
  block[62] = (bits >> 8) & 0xffu;
  block[63] = bits & 0xffu;
...
__device__ inline void ntlm_hash(const uint8_t* data, uint32_t len, uint32_t* out4) {
  if (len > 27) {  // UTF-16 doubles length, so 27*2=54 bytes max
    // Multi-block not implemented - return zero hash  
    out4[0] = out4[1] = out4[2] = out4[3] = 0;
    return;
  }
  uint16_t utf16[64];
  for (uint32_t i = 0; i < 64; ++i) utf16[i] = 0;
  for (uint32_t i = 0; i < len && i < 64; ++i) {
    utf16[i] = (uint16_t)data[i];
  }
  ...
  uint32_t utf16_len = len * 2;
  for (uint32_t i = 0; i < utf16_len && i < 56; ++i) {
    block[i] = ((uint8_t*)utf16)[i];
  }
  block[utf16_len] = 0x80u;
  
  uint64_t bits = (uint64_t)utf16_len * 8u;
  for (int i = 0; i < 8; ++i) block[56+i] = (bits >> (8*i)) & 0xffu;
...
__device__ __constant__ uint32_t g_target_hash[5] = {0};
__device__ __constant__ bool g_target_set = false;

__device__ inline bool check_hash(const uint32_t* digest) {
  if (!g_target_set) return false;
  return (digest[0] == g_target_hash[0] && 
          digest[1] == g_target_hash[1] && 
          digest[2] == g_target_hash[2] && 
          digest[3] == g_target_hash[3]);
}
